  ...
  return (
    ...
    >
      <div className="max-w-lg w-full p-8 border border-white/15 bg-[#0A0A0A]">
        <div className="mb-8">
          <h2 className="text-3xl sm:text-4xl font-bold tracking-tight mb-2 text-white" style={{ fontFamily: "'Outfit', sans-serif" }}>
            Find Your Path
          </h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            <label className="block text-xs font-mono tracking-widest uppercase mb-2 text-white/70">Full Name</label>
          ...
          <div>
            <label className="block text-xs font-mono tracking-widest uppercase mb-2 text-white/70">Email Address</label>
            <input type="email" name="email" value={formData.email} onChange={handleChange} required
              className="bg-transparent border border-white/15 text-white font-mono px-4 py-3 focus:outline-none focus:border-white rounded-none w-full" />
          </div>
          <div>
            <label className="block text-xs font-mono tracking-widest uppercase mb-2 text-white/70">Education</label>
            <select name="education" value={formData.education} onChange={handleChange} required
              ...
              {EDUCATION_OPTIONS.map(opt => <option key={opt.value} value={opt.value} className="bg-[#141414]">{opt.label}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-xs font-mono tracking-widest uppercase mb-2 text-white/70">Career Interest</label>
            <select name="domain" value={formData.domain} onChange={handleChange} required
              className="bg-transparent border border-white/15 text-white font-mono px-4 py-3 focus:outline-none focus:border-white rounded-none w-full appearance-none cursor-pointer">
              <option value="">Select your interest</option>
              {DOMAINS.map(domain => <option key={domain.value} value={domain.value} className="bg-[#141414]">{domain.label}</option>)}
            </select>
          </div>
